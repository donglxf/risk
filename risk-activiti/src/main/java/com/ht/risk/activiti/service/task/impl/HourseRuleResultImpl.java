...
public class HourseRuleResultImpl implements HourseRuleResult {
    ...
    public void execute(DelegateExecution execution) throws Exception {
        ...
        if (ActivitiConstants.EXCUTE_TYPE_SERVICE.equals(modelType)) {// 服务类型
            LOGGER.info("htapp HouseInfo channelType:>>>>>>"+execution.getVariable(ActivitiConstants.PROC_CHANNEL_TYPE));
            if ("CLS_APP".equals(execution.getVariable(ActivitiConstants.PROC_CHANNEL_TYPE))) {
                topicSenderService.sendHtApp(JSON.toJSONString(modelResult));
            } else {
